  ...
  try {
    if (!req.body.userId) {
      if (req?.body?.mediaImageUrl) {
        const imageUrl = Array.isArray(req.body.mediaImageUrl) ? req.body.mediaImageUrl[0]?.path : req.body.mediaImageUrl;
        if (imageUrl) deleteFromStorage(imageUrl);
      }
      if (req?.body?.mediaVideoUrl) {
        const videoUrl = Array.isArray(req.body.mediaVideoUrl) ? req.body.mediaVideoUrl[0]?.path : req.body.mediaVideoUrl;
        if (videoUrl) deleteFromStorage(videoUrl);
      }
    ...
    const storyData = {
      ...
      reactionsCount: 0,
      expiresAt,
      sharedContentKind: req.body?.sharedContentKind || null,
      sharedContentId: req.body?.sharedContentId || null,
      sharedContentPreviewUrl: req.body?.sharedContentPreviewUrl || "",
      sharedContentAuthorName: req.body?.sharedContentAuthorName || "",
      sharedContentAuthorImage: req.body?.sharedContentAuthorImage || "",
      sharedContentCaption: req.body?.sharedContentCaption || "",
    };
        ...
        $group: {
          ...
          stories: {
            $push: {
              ...
              isFake: "$isFake",
              createdAt: "$createdAt",
              sharedContentKind: "$sharedContentKind",
              sharedContentId: "$sharedContentId",
              sharedContentPreviewUrl: "$sharedContentPreviewUrl",
              sharedContentAuthorName: "$sharedContentAuthorName",
              sharedContentAuthorImage: "$sharedContentAuthorImage",
              sharedContentCaption: "$sharedContentCaption",
            },
          },
        },
      },
    ...
    const stories = await Story.aggregate([
      ...
      },
      {
        $project: {
          ...
          sharedContentKind: 1,
          sharedContentId: 1,
          sharedContentPreviewUrl: 1,
          sharedContentAuthorName: 1,
          sharedContentAuthorImage: 1,
          sharedContentCaption: 1,
          backgroundSong: {
            _id: "$backgroundSong._id",
            songTitle: "$backgroundSong.songTitle",
            songImage: "$backgroundSong.songImage",
