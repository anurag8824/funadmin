...
exports.deleteComment = async (req, res) => {
  try {
    if (!req.query.userId || !req.query.commentId) {
      return res.status(200).json({ status: false, message: "Oops ! userId and commentId are required." });
    }
    const userId = new mongoose.Types.ObjectId(req.query.userId);
    const commentId = new mongoose.Types.ObjectId(req.query.commentId);

    const comment = await PostOrVideoComment.findOne({ _id: commentId }).lean();
    if (!comment) {
      return res.status(200).json({ status: false, message: "Comment not found." });
    }
    if (!comment.userId || !comment.userId.equals(userId)) {
      return res.status(200).json({ status: false, message: "You can only delete your own comment." });
    }
