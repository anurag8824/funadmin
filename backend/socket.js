...
io.on("connection", async (socket) => {
  ...
  // ==================== WebRTC Audio Call Signaling Events ====================

  // Initiate audio call
  socket.on("audio-call-initiate", async (data) => {
    try {
      const parseData = typeof data === "string" ? JSON.parse(data) : data;
      console.log("audio-call-initiate:", parseData);

      const { callId, callerId, receiverId, callerInfo } = parseData;
      ...
      // Emit to receiver
      io.to(receiverRoom).emit("audio-call-incoming", {
        callId,
        callerId,
        receiverId,
        callerInfo,
      ...
      console.log(`Audio call initiated: ${callId} from ${callerId} to ${receiverId}`);
    } catch (error) {
      console.error("Error in audio-call-initiate:", error);
    }
  });

  // Answer audio call
  socket.on("audio-call-answer", async (data) => {
    try {
      const parseData = typeof data === "string" ? JSON.parse(data) : data;
      console.log("audio-call-answer:", parseData);

      const { callId, callerId, receiverId } = parseData;

      if (!callId || !callerId || !receiverId) {
        return;
      }

      // Get caller's socket room
      const callerRoom = `globalRoom:${callerId}`;
      
      // Emit to caller
      io.to(callerRoom).emit("audio-call-answered", {
        callId,
        callerId,
        receiverId,
      });
      ...
      console.error("Error in audio-call-answer:", error);
    }
  });

  // Reject audio call
  socket.on("audio-call-reject", async (data) => {
    try {
      const parseData = typeof data === "string" ? JSON.parse(data) : data;
      console.log("audio-call-reject:", parseData);
      ...
      // Emit to caller
      io.to(callerRoom).emit("audio-call-rejected", {
        callId,
        callerId,
        receiverId,
      });

      console.log(`Audio call rejected: ${callId}`);
    } catch (error) {
      console.error("Error in audio-call-reject:", error);
    }
  });

  // End audio call
  socket.on("audio-call-end", async (data) => {
    try {
      const parseData = typeof data === "string" ? JSON.parse(data) : data;
      console.log("audio-call-end:", parseData);

      const { callId, callerId, receiverId, endedBy } = parseData;

      if (!callId || !callerId || !receiverId) {
        return;
      }

      // Get both users' socket rooms
      const callerRoom = `globalRoom:${callerId}`;
      const receiverRoom = `globalRoom:${receiverId}`;
      
      // Emit to both users
      io.to(callerRoom).emit("audio-call-ended", {
        callId,
        callerId,
        receiverId,
        endedBy,
      ...
      io.to(receiverRoom).emit("audio-call-ended", {
        callId,
        callerId,
        receiverId,
        endedBy,
      ...
      console.log(`Audio call ended: ${callId} by ${endedBy}`);
    } catch (error) {
      console.error("Error in audio-call-end:", error);
    }
  });

  // WebRTC Offer
  socket.on("audio-call-offer", async (data) => {
    try {
      const parseData = typeof data === "string" ? JSON.parse(data) : data;
      console.log("audio-call-offer:", parseData.callId);

      const { callId, offer, callerId, receiverId } = parseData;

      if (!callId || !offer || !callerId || !receiverId) {
        return;
      }
      ...
      io.to(receiverRoom).emit("audio-call-offer-received", {
        callId,
        offer,
        callerId,
        receiverId,
      ...
      console.error("Error in audio-call-offer:", error);
    }
  });

  // WebRTC Answer
  socket.on("audio-call-answer-webrtc", async (data) => {
    try {
      const parseData = typeof data === "string" ? JSON.parse(data) : data;
      console.log("audio-call-answer-webrtc:", parseData.callId);

      const { callId, answer, callerId, receiverId } = parseData;

      if (!callId || !answer || !callerId || !receiverId) {
        return;
      }
      ...
      io.to(callerRoom).emit("audio-call-answer-received", {
        callId,
        answer,
        callerId,
        receiverId,
      ...
      console.error("Error in audio-call-answer-webrtc:", error);
    }
  });

  // ICE Candidate
  socket.on("audio-call-ice-candidate", async (data) => {
    try {
      const parseData = typeof data === "string" ? JSON.parse(data) : data;

      const { callId, candidate, fromUserId, toUserId } = parseData;

      if (!callId || !candidate || !fromUserId || !toUserId) {
        return;
      }

      // Get target user's socket room
      const targetRoom = `globalRoom:${toUserId}`;
      
      // Emit ICE candidate to target user
      io.to(targetRoom).emit("audio-call-ice-candidate-received", {
        callId,
        candidate,
        fromUserId,
        toUserId,
      ...
      console.error("Error in audio-call-ice-candidate:", error);
    }
  });
