...
function isProtectedDefaultPhoto(fileUrl) {
  const pathname = new URL(fileUrl).pathname;
  return pathname.includes("/defaultphoto/");
}
...
const deleteFromStorage = async (fileUrl, forceDeleteDefault = false) => {
  try {
    if (!fileUrl) return;

    if (isProtectedDefaultPhoto(fileUrl) && !forceDeleteDefault) {
      console.warn("⚠️ Protected file (defaultphoto) — skipping deletion:", fileUrl);
      return;
    }
